...
For `.go` files, apply `kennedy-go`, `kennedy-if`,
`kennedy-arch`, and `kennedy-ext` (the last only when
...
branch chains that fit a defaulting or naked switch (per `kennedy-if`);
